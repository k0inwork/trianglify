...
function Trianglify(opts) {
  ...
  for (var i=0; i < geom_indices.length; i += 3) {
    ...
    var centroid = f(vertices);
    var color = gradient(norm_x(centroid.x), norm_y(centroid.y)).css();
    //gradient color
    
    var p1 = [vertices[0][0],vertices[0][1]];
    var color1 = gradient(norm_x(p1[0]),norm_y(p1[1]));
    var p2 = [(vertices[1][0]+vertices[2][0])/2,(vertices[1][1]+vertices[2][1])/2];
    var color2 = gradient(norm_x(p2[0]),norm_y(p2[1]));
    
   var grad = [color1,color2];
    
    triangles.push([color, vertices,grad]);
  }
